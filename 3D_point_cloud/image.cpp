...
void Image::set_pixel(int x, int y, int c, unsigned char val)
{
	if (x >= width || x < 0 || y >= height || y < 0 || c >= channels || c < 0) {
		std::cerr << "set_pixel() error: Index out of bounds.\n";
		std::exit(1);
...
unsigned char Image::get_pixel(int x, int y, int c) const
{
	if (x < 0)
		x = 0;
	if (x >= width)
...
Image gaussian_blur(const Image& img, float sigma)
{
	...
	float* kernelr = (float*)malloc(sizeof(float) * (size));

	float sum = 0;
	for (int k = -size / 2; k <= size / 2; k++) {
		float val = std::exp(-(k*k) / (2 * sigma*sigma));
		//kernel.set_pixel(center + k, 0, 0, val); 
		kernelr[center + k] = val;
		sum += val;
		//printf("%.5f %.5f\n", val,sum);
	}
	for (int k = 0; k < size; k++) {
		//float ff = kernel.data[k];
		kernelr[k] /= sum;
		//printf("%f \n", kernelr[k]);
	}
	...
	for (int x = 0; x < img.width; x++) {
		for (int y = 0; y < img.height; y++) {
			...
			for (int k = 0; k < size; k++) {
				int dy = -center + k;
				int dd = img.get_pixel(x, y + dy, 0) * kernelr[k];// kernel.data[k];
				sum += img.get_pixel(x, y + dy, 0) * kernelr[k];// kernel.data[k];
			}
			tmp.set_pixel(x, y, 0, sum);
		}
	}
	...
	for (int x = 0; x < img.width; x++) {
		for (int y = 0; y < img.height; y++) {
			...
			for (int k = 0; k < size; k++) {
				int dx = -center + k;
				sum += tmp.get_pixel(x + dx, y, 0) * kernelr[k];// kernel.data[k];
			}
			filtered.set_pixel(x, y, 0, sum);
		}
	}
